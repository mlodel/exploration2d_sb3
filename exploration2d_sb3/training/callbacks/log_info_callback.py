...
class LogInfoCallback(BaseCallback):
    ...
    def _on_step(self) -> None:
        ...
        for key in self.info_keys:
            self.info_buffer[key][self.step, :] = np.array(
                [info[key] for info in infos], dtype=np.float32
            )
    ...
    def _on_rollout_end(self) -> None:
        ...
        n_episodes = np.sum(np.sum(self.info_buffer["n_episodes"], axis=0))
        self.logger.record(
            "rollout/n_episodes",
            n_episodes,
        )
        ...
        for key in self.info_keys:
            self.logger.record(
                "rollout/" + key,
                np.sum(np.sum(self.info_buffer[key], axis=0)) / n_episodes,
            )
